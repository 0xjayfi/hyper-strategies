...
You are running an automated content pipeline. Your job is to generate a wallet spotlight post for X (Twitter) and push it to Typefully as a draft for review.

## Step 1: Load context

Read these files:
- `data/content_payload_wallet_spotlight.json` — the signal data (wallet, score change, dimensions, current live positions with uPnL)
...
- data/content_payload_wallet_spotlight.json
...
- Lead with the rank/score change. What moved in the composite score and leaderboard position? Start with the interesting delta, then unpack which dimensions drove it.
- Include dimension analysis: which of the 6 dimensions moved, by how much, and what that means for the trader's behavior. Don't list all 6 if only 2-3 are relevant. Focus on what changed and why it matters.
- Interpret the data: what does it SIGNAL? (overexposure, risk blowup, conviction loss, discipline improvement, leverage reduction, etc.)
...
The ONLY valid screenshot filenames are: `leaderboard_top5.png`, `trader_scoring.png`, `trader_positions.png`. If the draft uses any other filename, replace it with the correct one from this list.
...
- Does the thread lead with the rank/score change before diving into dimension analysis?
...
- Do NOT change screenshot filenames. The only valid filenames are: `leaderboard_top5.png`, `trader_scoring.png`, `trader_positions.png`.
...
- data/content_payload_wallet_spotlight.json (the source data)
...
- Are the screenshot filenames valid? Only these three are allowed: `leaderboard_top5.png`, `trader_scoring.png`, `trader_positions.png`. Any other filename is invalid and must be corrected.
...
The valid screenshot files for this angle are: `leaderboard_top5.png`, `trader_scoring.png`, `trader_positions.png`.

For each screenshot, check:

**Relevance:**
- Does the screenshot content match the tweet it's attached to? A scoring screenshot should appear with score discussion, a positions screenshot with positions discussion, a leaderboard screenshot with leaderboard context.
